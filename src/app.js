...
app.get("/estabelecimento", (req, res) => {
  Estabelecimento.findById(req.query.id, {
    impressora: 0,
    integracao: 0,
    painel: 0,
    configPedidos: 0,
...
app.post("/start-rotina-validacao-tempo-teste", async (req, res) => {
  try {
    ...
    let usuariosMaster = await Usuario.find({
      $and: [{ statusAtivo: true }, { "freeSystem.habilitado": true }],
    });
    ...
    if (
      ...
      registerLog.registerLog({
        text: "Rotina de encerramento periodo teste",
        code: "200",
        description: `Nenhum usuário para encerrar o període de teste`,
      });
    } else {
      usuariosMaster.forEach((usuario) => {
        if (moment().diff(usuario.freeSystem.dataFim, "days") <= 0) {
          registerLog.registerLog({
            text: "Rotina de encerramento periodo teste",
            code: "200",
            description: `Usuario ${usuario._id} ainda possui período de teste`,
          });
        } else {
          usuario.estabelecimentosVinculados.forEach(
            async (estabelecimento) => {
              try {
                ...
                await Usuario.updateMany(
                  {
                    "estabelecimentosSelecionados.idEstabelecimento":
                ...
                registerLog.registerLog({
                  text: "Rotina de encerramento periodo teste",
                  code: "200",
                  description: `Estabelecimento ${estabelecimento.idEstabelecimento} acabou o periodo de teste`,
                });
              } catch (err) {
                return registerLog.registerLog({
                  text: "Error in the test system change routine",
                  code: "500",
                  description: `Ocorreu um erro na rotina de encerramento do periodo de teste, err: ${err}`,
                });
              }
            }
          );
    ...
    return registerLog.registerLog({
      ...
      code: "500",
      description:
        "Ocorreu um erro ao executar a rotina do sistema, favor analisar o log e consultar qual estabelecimento foi parado",
    });
  }
});
...
app.post("/start-rotina-gerar-cobranca", async (req, res) => {
  try {
    ...
    if (rotina.length === 0) {
      ...
      return registerLog.registerLog({
        text: "No routine to run",
        code: "200",
        description: "A base de dados não possui nenhuma rotina para executar",
      });
    }
      ...
      try {
        switch (rotina.tipo) {
          case "cobranca":
                ...
                $push: {
                  "locacao.faturas": {
                    $each: [
                      {
                        idPlano: plano._id,
                        descricao: `Fatura ${moment(
                          rotina.typeCobranca.vencimento
                        ).format("DD/MM/YYYY")} até ${moment(
                          rotina.typeCobranca.vencimento
                        )
                          .add(plano.mesesPeriodicidade, "months")
                          .format("DD/MM/YYYY")}`,
                        valor: plano.valor,
                        ...
                        idTransacaoOperadora: `${
                          rotina.typeCobranca.idEstabelecimento
                        }#@#${uuidv4() + uuidv4()}`,
                        logs: [
                          {
            ...
            registerLog.registerLog({
              text: "Charge generated",
              code: "200",
              description: `Cobrança gerada para o estabelecimento ${rotina.typeCobranca.idEstabelecimento}`,
            });
            break;
  ...
  try {
    ...
    let estabelecimentos = await Estabelecimento.find({
      $and: [
        ...
        {
          "locacao.dataLiberado": {
            $lt: moment().subtract(1, "days").format("YYYY-MM-DDT23:59"),
          },
        },
      ],
    });
    ...
    if (estabelecimentos.length === 0) {
      registerLog.registerLog({
        text: "Rotina de inativar estabelecimento",
        code: "500",
        description: "Não teve nenhum estabelecimento para inativar",
      });
    }
    ...
    registerLog.registerLog({
      text: "Erro na rotina de remoção dos estabelecimentos",
      code: "500",
      description: err,
    });
  }
